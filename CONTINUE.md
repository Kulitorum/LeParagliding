...
`SimBody::brakeApplied` chases the control at `kControlHandSpeed` (0.6 m/s)
...
### Authored riser-row controls — done

The Playground exposes one symmetric pull slider for each authored non-brake
row present at the carabiner cut (plans 1..5 become A..E). A normal three- or
four-row glider therefore shows A/B/C or A/B/C/D; an old mesh without row-plan
tags shows no riser controls and must be regenerated by the engine.

These independent symmetric row pulls are diagnostic controls, not a parakite
pulley mixer. A parakite steering input changes several row lengths together
on one half so that the half-wing changes incidence while its trailing edge
stays clean. That needs an explicit per-side mixer schedule/pulley geometry;
calling one independent A/B/C/D slider a parakite brake would misdescribe both
the mechanism and its leverage.

The input shortens only the bottom cable segment of that row on each side,
immediately above the carabiners. This is the same physical cut used by the
row-load instrument. Shortening every same-plan branch would multiply a single
hand movement through every cascade level, so it is intentionally not done.
The XPBD cable change carries force through the authored suspension and the
pilot/harness constraints; there is no imposed pitch, aerodynamic trim or
recovery force. Pull and release use the brake's 0.6 m/s simulated-time hand
speed, travel is bounded to 30 cm, and Reset releases every row before the body
is rebuilt. `ensureView` explicitly pushes all row defaults, while a row that
disappears on the next loaded glider is cleared before its hidden slider can
retain a stale pull.
...
pose begins at exactly zero Green strain.

The same material mode now replaces each rib ladder bay's five bilateral
struts/rails/diagonal with two orthotropic membrane faces. Rib warp follows the
section chord and weft lies across its depth. The rib outline remains sewn to
the skin, but the shared triangle diagonal receives no hinge, so the web can
wrinkle and fold instead of acting as an in-plane rigid triangulation. Faces
below `1e-7 m2` are counted and omitted as numerical closing slivers; they are
less than 0.1 mm2 and otherwise make the constitutive solve ill-conditioned.
Straps, seams, suspension, cells and the opt-in contact path are retained.
The first implementation is structural: it does not yet stamp the difference
between neighbouring cell pressures onto the internal rib faces. That term is
near zero in a uniformly inflated wing but can matter during an asymmetric
collapse, so recovery conclusions must keep this limitation visible.
...
Every valid interior **skin** two-triangle edge receives a signed four-node
XPBD dihedral hinge. It resists only fold, not in-plane shear, is mass weighted
and solved after the membrane sweep. Zero workers retain serial insertion
order; threaded runs use a deterministic node-disjoint four-node colouring
whose full-wing checksum is identical at one, four and six requested workers.
Rib diagonals and skin boundaries stay free; degenerate, non-manifold and
inconsistently wound incidence is counted and skipped. Hinge state participates
in rollback and persistence.
...
The native gnuC2 material inventory is 17,374 membranes (1,534 rib), 23,727
skin hinges, 52 omitted rib closing slivers, and 4,631 retained seam/strap/
distance/cable constraints. A bounded final-Cp settle at 80 Pa completed with
no shape flags at span +1.44%, area +0.62%, volume +3.09%, 69.2 mm worst
leading-edge dent and -2.07 degrees worst twist. With six requested workers it
averaged 142 ms/frame while settling; it is a structural prototype, not a
calibrated absolute-aerodynamics claim. The cheaper hard-load probe is:

```powershell
.\build\bin\Release\softwing-bench.exe build\aero\gnuC2\lep-sim.json --membrane --frames 10 --warmup 0
```

It also remains finite at the full 80 Pa hard load. Before the solver follow-up,
a six-worker material frame cost about 184 ms: 52 ms membrane, 98 ms serial
skin hinges and 10 ms solver diagnostics. Node-disjoint hinge colouring and
omitting the unused residual/resultant diagnostic pass reduce repeated
three-frame measurements to 113--131 ms: 50 ms membrane, 40--46 ms hinges and
zero diagnostic-pass time. Full-wing checksums match exactly with one, four
and six requested workers. These timings are engineering observations, not
performance guards.
...
skin/rib membrane and line network at the high substep ratio. It is not the
compression branch or hinge path: bilateral compression with nearly disabled bending shows
...
hinges, fabric-rib inventory/rest state/free folds, degeneracies/non-manifold
incidence and deterministic finite stepping.
...
## Laptop restart handoff — 2026-08-12

The current `main` worktree contains one coherent, **uncommitted** Playground
change set:

- symmetric diagnostic A--E riser-row sliders, acting on the authored bottom
  cable at the carabiner cut;
- the opt-in `Orthotropic fabric: skin + ribs (prototype)` formulation, with
  actual membrane rib webs and freely folding rib diagonals;
- deterministic parallel skin-hinge colouring and omission of the unused
  stored membrane residual/resultant pass in Playground;
- focused tests, benchmark reporting, UI/session-log text and the matching
  design records.

Do not include or remove these unrelated untracked local files when committing:

```text
Data/Hegala-v2-edit/
Data/gnuC2.txt
Data/my-wing-saved-setup.txt
tests/fixtures/3.28/lep-sim.json
```

Last verification on the Windows desktop:
...
The build succeeded and all 27 configured tests passed. The legacy gnuC2
`--shape --csv --legacy-pressure` row remained unchanged. The material gnuC2
80 Pa settle completed without shape flags at about 142 ms/frame with six
requested workers; a short threaded material free-flight launch also remained
finite. Rebuild from the laptop's own build tree before testing because no
generated build products belong to this handoff.

The next meaningful interactive check is Swoop Original in free flight with
the material skin+ribs selector: repeat symmetric stall/release and individual
A--D row pulls, inspect detailed rib strain, and record whether recovery or
rib deformation improves. The independent row sliders are diagnostic—not yet
